...
| Claude Code CLI (`claude-cli`, port addition) | `claude -p --output-format stream-json` — same SSE events as the Messages API, wrapped per line | `--tools WebSearch --allowedTools WebSearch`; tool_use blocks surface as WebSearch events |
| Codex CLI (`codex-cli`, port addition) | `codex exec --json` — whole items, no deltas | `-c web_search="live"`; `web_search` items surface as WebSearch events |

**Installed-CLI providers** (`Providers/ClaudeCliProvider`, `CodexCliProvider`, `CliTools`, `CliRunner`): instead of an API key, the app shells out to the Claude Code or Codex CLI the user already has signed in. One process per query: prompt on stdin (never the command line — Windows caps it at 32 K), system prompt via `--system-prompt` for Claude (folded into stdin past 24 K) and as an `<instructions>` block for Codex (which has no such flag); JSONL stdout parsed line by line. Both run in an empty scratch folder under the app's local-data dir so no CLAUDE.md / AGENTS.md / project settings leak in; Codex is sandboxed read-only and ephemeral, Claude has every agent tool disabled except WebSearch in web mode. Temperature is not a CLI option and is dropped. Cost: Claude's `result` line reports `total_cost_usd` (list-price basis); Codex reports tokens only, priced through the OpenAI table. Chat sessions render the transcript as one prompt (`ChatProvider.RenderTranscript`) since there is no session to resume. Detection (`CliTools.FindClaude/FindCodex`) runs on every use: explicit path → PATH → installer locations; the Codex desktop app moves its bundled binary to a new hashed folder on each update, so the newest `bin\**\codex.exe` wins. `ICliRunner` is the test seam; `ProcessCliRunner` is the real one.
