...
- Fetching runs the four-step resolution in 05 §5.7 (Workday CXS · Greenhouse · iCIMS · ADP WorkforceNow board APIs · plain HTTP behind the SSRF guard · JSON-LD/OpenGraph in that HTML · one LLM extraction). Progress and fallback cost show inline; on failure, the "paste the text instead" message and the URL is left untouched.
