...
- Fetching runs the four-step resolution in 05 §5.7 (Workday CXS · Greenhouse · iCIMS · ADP WorkforceNow · SmartRecruiters board APIs · plain HTTP behind the SSRF guard · JSON-LD/OpenGraph in that HTML · one LLM extraction). Progress and fallback cost show inline; on failure, the "paste the text instead" message and the URL is left untouched.
...
- **Shortcuts** (Windows only; the group is hidden elsewhere): Desktop / Start menu rows, one button each. A button reads **Add shortcut** until that place holds a `.lnk` whose target is *this* executable's path, then **Remove shortcut** — a shortcut to another copy of the app doesn't count and is never touched (the new one takes a numbered name beside it). Both are written by the app (`Platform/ShellLink` writes the `.lnk` in managed code; COM is unavailable under the trimmed publish). There is no taskbar row: Windows 11 ignores the shell's hidden pin verb from any process but Explorer and the pin API needs package identity, so the app cannot pin, and a button that only explained that was dropped.
